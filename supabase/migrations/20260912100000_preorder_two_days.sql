-- Preorders use the next two local calendar days, not a rolling 24-hour delay.
...
begin
  ...
  if not found or p_customer_id is null or p_request_id is null or p_scheduled_at is null
    or (p_scheduled_at at time zone 'Asia/Aqtau')::date not in ((now() at time zone 'Asia/Aqtau')::date + 1, (now() at time zone 'Asia/Aqtau')::date + 2)
    or p_items is null or jsonb_typeof(p_items)<>'array' or jsonb_array_length(p_items) not between 1 and 100 then
    raise exception 'Предзаказ доступен только на завтра и послезавтра' using errcode='22023';
  end if;
