-- Keep pairing-code replacement and redemption in the same lock order.
-- Previously, replacing a code during activation could deadlock branch/code locks.
...
create or replace function public.activate_pos_device(p_code_hash text,p_terminal uuid,p_group uuid,p_name text,p_token_hash text,p_expected_branch uuid default null)
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
...
begin
  ...
  -- Issuance locks branch then code; activation must use the same order.
