-- Display purchases require a counted branch balance, including late payment
-- callbacks. Preorders remain stop-list-only and completed allocations stay idempotent.
...
begin
  ...
  loop
    ...
    if not found then
      raise exception 'Товар «%» сейчас недоступен: остаток не указан',
        coalesce(inventory_row.product_name, item_id) using errcode='P0001';
    end if;
    ...
    if found then
      ...
      if inventory_row.source_quantity is null then
        raise exception 'Товар «%» сейчас недоступен: остаток не указан',
          coalesce(inventory_row.product_name, item_id) using errcode='P0001';
      end if;
...
begin
  ...
  loop
    ...
    if inventory_row.source_quantity is null then
      return jsonb_build_object('status','unavailable','reason','inventory_unknown',
        'productId',requested_product.product_id);
    end if;
    if inventory_row.manual_stop then
      return jsonb_build_object(
