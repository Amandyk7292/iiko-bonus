-- Preserve the last absolute count when an additive cashier arrival follows an offline sale.
...
begin
  ...
  -- An arrival is additive: older offline sales still need to be subtracted.
  -- Only an absolute correction establishes a new physical-count watermark.
  result:=public.update_cashier_inventory_before_receipts(
    p_branch_id,p_product_id,p_product_name,effective_revision,clean_changes
  );
  if requested_reason='correction' or stock_was_missing then
    update public.branch_product_inventory set manual_counted_at=now()
      where branch_id=p_branch_id and product_id=p_product_id;
  end if;
  select * into stock from public.branch_product_inventory
    where branch_id=p_branch_id and product_id=p_product_id;
