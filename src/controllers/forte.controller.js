...
const availability = async (_req, res) => {
  const available = forteService.availability();
  res.json({
    success: true,
    available,
...
const createPayment = async (req, res) => {
  try {
    ...
    const result = await checkoutRequests.run(requestKey, async () => {
      ...
      try {
        const payment = await forteService.createCheckout(
          phone,
          pricing,
          customerId,
          {
        ...
        const order = await forteService.existingRequest(customerId, checkoutId);
        if (order?.id) {
        ...
        const order = await forteService.existingRequest(customerId, checkoutId).catch(() => null);
        if (!order) {
...
const checkStatus = async (req, res) => {
  try {
    ...
    if (!PAYMENT_TOKEN_PATTERN.test(operationId)) {
      return res.status(400).json({ error: 'Некорректный operationId' });
    }
    const customerId = req.customerAuth.id;
    let order = await forteService.getOrderStatus(operationId, customerId);
    if (order.status === 'paid') {
      order = (await forteService.orderService.recordPaidOrder(operationId)) || order;
    } else if (!['failed', 'expired', 'refunded'].includes(order.status)) {
      try {
        await forteService.syncOrder(order);
        order = await forteService.getOrderStatus(operationId, customerId);
      } catch (error) {
