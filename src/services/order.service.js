...
async function loadOrderCatalog({ branchId = null, orderType = 'pickup' } = {}) {
  ...
  const [stopIds, productOverrides, categoryOverrides, customProducts] = await Promise.all([
    ...
    menuService.getProductOverrides({ strict: true, profileKey: selectedIikoApi.profileKey }),
    menuService.getCategoryOverrides({ strict: true, profileKey: selectedIikoApi.profileKey }),
    menuService.getCustomProducts({ strict: true, profileKey: selectedIikoApi.profileKey }),
  ]);
