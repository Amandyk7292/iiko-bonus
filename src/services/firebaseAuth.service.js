  ...
  try {
    ...
    if (!signData.idToken) {
      if (signData.error && signData.error.message === 'CONFIGURATION_NOT_FOUND') {
        console.warn(
          'Firebase Authentication не включен в консоли Firebase. Включите Email/Password провайдер в консоли Firebase (Authentication -> Sign-in method).',
        );
        return false;
      }
    ...
    if (oobData.error) {
      console.warn('Ошибка отправки подтверждения почты Firebase:', oobData.error.message);
      return false;
    }

    console.log(`Письмо подтверждения почты Firebase успешно отправлено на ${email}`);
    return true;
  } catch (err) {
