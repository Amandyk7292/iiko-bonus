...
export default function StoriesPage() {
  ...
  const openModal = (story?: any) => {
    ...
    setFormError('');
    if (!story) {
      const nextOrder = stories.length
  ...
  const updateField = (language: ContentLanguage, field: keyof LocalizedStory, value: string) => {
    setI18n((current) => ({ ...current, [language]: { ...current[language], [field]: value } }));
    setFormError('');
  };
  ...
  const save = async (event: FormEvent) => {
    ...
    const missingRequiredField = [
      {
        ...
        message: t('stories.validationTitle'),
    ...
    if (missingRequiredField) {
      setActiveLanguage('ru');
      setFormError(missingRequiredField.message);
      window.requestAnimationFrame(() =>
        document.getElementById(missingRequiredField.focusId)?.focus(),
      );
      return;
    }
    setSubmitting(true);
  ...
  const remove = async (story: any) => {
    ...
    try {
      await api.deleteStory(String(story.id));
  ...
  if (error && stories.length === 0)
    return <PageState type="error" description={error} onRetry={fetchStories} />;
    ...
    <div className="page-stack">
      ...
      {stories.length === 0 ? (
        <PageState
          ...
          description={t('stories.emptyHint')}
          action={
        ...
        <section className="card table-card">
          <div className="responsive-table-wrap">
            <table className="data-table stories-table">
              <thead>
                <tr>
                  <th scope="col">{t('stories.illustration')}</th>
                  <th scope="col">{t('stories.copy')}</th>
                  <th scope="col">{t('stories.languages')}</th>
              ...
              <tbody>
                {stories.map((story) => {
                  ...
                  return (
                    <tr key={story.id}>
                      <td data-label={t('stories.illustration')}>
                        <div className="story-thumbnail">
                          {image ? (
                            <img
                              src={image}
                              alt={localized.title || story.title || t('stories.illustration')}
                              width="112"
                              height="60"
                              loading="lazy"
                            />
                      ...
                      <td data-label={t('stories.copy')}>
                        <strong>{localized.title || story.groupTitle || story.title}</strong>
                        ...
                        <div className="language-badges">
                          {languages.map((language) => (
                            <span
                              ...
                              className={
                                data[language]?.title
                                  ? 'language-badge language-badge-complete'
                                  : 'language-badge'
                              }
                            >
                              {t(`content.${localeKey(language)}`)}
                            </span>
                          ))}
                        </div>
                      </td>
                      <td data-label={t('common.order')} className="text-center tabular">
                        {Number(story.sortOrder) || 0}
                      </td>
        ...
        <form className="modal-body form-stack" onSubmit={save}>
          ...
          <div className="form-grid form-grid-2">
            <div className="field-group">
              <label className="field-label" htmlFor={`story-title-${activeLanguage}`}>
                {t('stories.promoTitle')} ({t(`content.${localeKey(activeLanguage)}`)})
                {activeLanguage === 'ru' ? ' *' : ''}
              </label>
              <input
                id={`story-title-${activeLanguage}`}
                className="input-classic"
                value={current.title}
                onChange={(event) => updateField(activeLanguage, 'title', event.target.value)}
                placeholder={placeholders[activeLanguage].title}
                required={activeLanguage === 'ru'}
                maxLength={160}
              />
            </div>
            <div className="field-group">
              <label className="field-label" htmlFor={`story-description-${activeLanguage}`}>
                {t('stories.promoDescription')} ({t(`content.${localeKey(activeLanguage)}`)})
              </label>
              <input
                id={`story-description-${activeLanguage}`}
                className="input-classic"
                value={current.description}
                onChange={(event) => updateField(activeLanguage, 'description', event.target.value)}
                placeholder={placeholders[activeLanguage].description}
                maxLength={300}
              />
            </div>
          </div>
          ...
          <div className="story-upload-grid">
            ...
            <ImageUpload
              ...
              required={activeLanguage === 'ru'}
              image={effectiveContent}
              fallback={activeLanguage !== 'ru' && !current.contentUrl}
              loading={uploadingField === `${activeLanguage}-contentUrl`}
              onFile={(event) => uploadFile(event, 'contentUrl')}
              onReset={
                current.contentUrl && activeLanguage !== 'ru'
                  ? () => updateField(activeLanguage, 'contentUrl', '')
                  : undefined
              }
              vertical
            />
          ...
          <div className="form-grid form-grid-2">
            ...
            <div className="field-group">
              <label className="field-label" htmlFor="story-order">
                {t('stories.sortOrder')}
              </label>
              <input
                id="story-order"
      ...
      <div className="field-group">
        <div className="upload-label-row">
          <span className="field-label icon-label">
            {vertical ? <Smartphone size={15} /> : <ImageIcon size={15} />}
            {title}
          </span>
          <small>{required ? t('common.required') : t('common.optional')}</small>
        </div>
        <label
          className={`story-upload ${vertical ? 'story-upload-vertical' : 'story-upload-horizontal'} ${image ? 'story-upload-filled' : ''}`}
        >
          <input
            id={inputId}
            type="file"
          ...
          {isUploading ? (
            ...
            <>
              <img
                ...
                width="1080"
                height={vertical ? 1920 : 480}
              />
              <span className="upload-overlay">
                <Upload aria-hidden="true" size={20} />
                {t('stories.replaceImage')}
              </span>
              {fallback && <small className="fallback-chip">{t('stories.fallbackImage')}</small>}
            </>
          ) : (
            <div className="upload-placeholder">
              <Upload aria-hidden="true" size={22} />
              <span>{t('stories.chooseImage')}</span>
            </div>
          )}
        </label>
        {onReset && (
