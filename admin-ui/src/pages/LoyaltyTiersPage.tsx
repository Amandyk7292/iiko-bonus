...
export default function LoyaltyTiersPage() {
  ...
  const openCreate = () => {
    const nextOrder = sortedTiers.length
      ? Math.max(...sortedTiers.map((tier) => tier.sortOrder)) + 1
      : 0;
    setEditingId(null);
  ...
  const validate = () => {
    if (!draft.code.trim() || !/^[a-z0-9_-]+$/i.test(draft.code.trim()))
      return t('tiers.validationCode');
    if (contentLocales.some((language) => !draft.names[language].trim()))
      return t('tiers.validationNames');
    if (
      draft.minSpend === '' ||
  ...
  const saveTier = async (event: FormEvent) => {
    ...
    const payload: LoyaltyTierInput = {
      ...
      names: Object.fromEntries(
        contentLocales.map((language) => [language, draft.names[language].trim()]),
      ) as unknown as LocalizedText,
      descriptions: Object.fromEntries(
        contentLocales.map((language) => [
          language,
          (draft.descriptions[language] || draft.names[language]).trim(),
        ]),
      ) as unknown as LocalizedText,
  ...
  const toggleTier = async (tier: LoyaltyTier) => {
    ...
    try {
      ...
      setTiers((current) =>
        current.map((item) => (item.id === tier.id ? { ...item, isActive: !item.isActive } : item)),
      );
  ...
  return (
    <div className="page-stack">
      ...
      {loading ? (
        <PageState type="loading" title={t('tiers.load')} />
      ) : error ? (
        ...
        <PageState
          ...
          action={
            <button type="button" onClick={openCreate} className="btn-classic px-5">
              {t('tiers.add')}
            </button>
          }
        />
                    ...
                    <tr key={tier.id} className={!tier.isActive ? 'row-muted' : undefined}>
                      <td data-label={t('tiers.name')}>
                        <div className="tier-name-cell">
                          ...
                          <div>
                            <strong>{tier.names[locale] || tier.names.ru}</strong>
                            <small>{tier.code}</small>
                          </div>
                        </div>
                      </td>
                      <td data-label={t('tiers.minSpend')} className="tabular">
                        {formatNumber(tier.minSpend)} ₸
                      </td>
                      <td data-label={t('tiers.cashback')}>
                        <strong className="cashback-value">
                          {formatNumber(tier.cashbackPercent, { maximumFractionDigits: 2 })}%
                        </strong>
                      </td>
                      <td data-label={t('common.status')}>
                        <span
                          className={`status-pill ${tier.isActive ? 'status-active' : 'status-inactive'}`}
                        >
                      ...
                      <td data-label={t('common.actions')}>
                        <div className="row-actions justify-end">
                          ...
                          <button
                            ...
                            className="icon-button"
                            onClick={() => moveTier(index, 1)}
                            disabled={index === sortedTiers.length - 1 || Boolean(busyId)}
                          ...
                          <button
                            ...
                            aria-label={tier.isActive ? t('tiers.deactivate') : t('tiers.activate')}
                            title={tier.isActive ? t('tiers.deactivate') : t('tiers.activate')}
                          >
                            {busyId === tier.id ? (
          ...
          <aside className="tier-preview-column" aria-label={t('common.preview')}>
            ...
            {sortedTiers
              ...
              .map((tier, index) => (
                <article
                  key={tier.id}
                  className="tier-preview-card tier-artwork"
                  style={
                    {
                      '--tier-index': index,
                      ...tierArtworkStyle(tier.code, tier.backgroundImageUrl),
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="tier-preview-meta">
                      <span>
                        {t('tiers.previewSpend', { amount: formatNumber(tier.minSpend) })}
                      </span>
                      <strong>
                        {t('tiers.previewCashback', {
                          percent: formatNumber(tier.cashbackPercent, { maximumFractionDigits: 2 }),
                        })}
                      </strong>
                    </div>
                  </div>
        ...
        <form onSubmit={saveTier} className="modal-body form-stack" noValidate>
          ...
          <div className="form-grid form-grid-2">
            <div className="field-group">
              <label className="field-label" htmlFor="tier-code">
                {t('tiers.code')} *
              </label>
              <input
                id="tier-code"
            ...
            <div className="field-group">
              <label className="field-label" htmlFor="tier-order">
                {t('common.order')}
              </label>
              <input
                id="tier-order"
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    sortOrder: event.target.value === '' ? '' : event.target.valueAsNumber,
                  }))
                }
              />
            </div>
            <div className="field-group">
              <label className="field-label" htmlFor="tier-spend">
                {t('tiers.minSpend')} *
              </label>
              <input
                id="tier-spend"
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    minSpend: event.target.value === '' ? '' : event.target.valueAsNumber,
                  }))
                }
                required
              />
            ...
            <div className="field-group">
              <label className="field-label" htmlFor="tier-cashback">
                {t('tiers.cashback')} *
              </label>
              <input
                id="tier-cashback"
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    cashbackPercent: event.target.value === '' ? '' : event.target.valueAsNumber,
                  }))
                }
                required
              />
          ...
          <div className="form-grid form-grid-2">
            <div className="field-group">
              <label className="field-label" htmlFor={`tier-name-${activeLanguage}`}>
                {t('tiers.name')} ({t(`content.${activeLanguage}`)}) *
              </label>
              <input
                id={`tier-name-${activeLanguage}`}
                className="input-classic"
                value={draft.names[activeLanguage]}
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    names: { ...current.names, [activeLanguage]: event.target.value },
                  }))
                }
                required
              />
            ...
            <div className="field-group">
              <label className="field-label" htmlFor={`tier-description-${activeLanguage}`}>
                {t('tiers.conditions')} ({t(`content.${activeLanguage}`)})
              </label>
              <textarea
                id={`tier-description-${activeLanguage}`}
                className="input-classic"
                rows={3}
                value={draft.descriptions[activeLanguage]}
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    descriptions: { ...current.descriptions, [activeLanguage]: event.target.value },
                  }))
                }
              />
