...
export default function AnalyticsPage() {
  ...
  const funnel: Record<string, number> = stats.funnel && typeof stats.funnel === 'object' ? stats.funnel : {};
  const bonusData = {
  ...
  const chartOptions = {
    ...
    plugins: { legend: { position: 'bottom' as const, labels: { padding: 16, font: { family: 'Golos Text', size: 12 } } } },
  };
  const branchData = {
    labels: branches.map((item: any) => item.branch),
    datasets: [{ label: t('analytics.branchRevenue'), data: branches.map((item: any) => Number(item.revenue || 0)), backgroundColor: '#b88c5a', borderRadius: 8 }],
  };
  ...
  return (
    <div className="page-stack">
      ...
      <section className="stats-grid stats-grid-secondary analytics-order-metrics">
        <article className="card metric-card"><p>{t('analytics.orders30')}</p><strong>{formatNumber(stats.paidOrdersLast30Days || 0)}</strong><small>{t('analytics.activeOrders', { count: formatNumber(stats.activeOrders || 0) })}</small></article>
        <article className="card metric-card"><p>{t('analytics.sales30')}</p><strong>{formatNumber(stats.salesLast30Days || 0)} ₸</strong><small>{t('analytics.averageOrder', { amount: formatNumber(stats.averageOrderValueLast30Days || 0) })}</small></article>
        <article className="card metric-card"><p>{t('analytics.refunds30')}</p><strong>{formatNumber(stats.refundsLast30Days || 0)}</strong><small>{formatNumber(stats.refundAmountLast30Days || 0)} ₸</small></article>
        <article className="card metric-card"><p>{t('analytics.completionTime')}</p><strong>{formatNumber(stats.averageCompletionMinutesLast30Days || 0)} {t('analytics.minutes')}</strong><small>{t('analytics.cancelled30', { count: formatNumber(stats.cancelledOrdersLast30Days || 0) })}</small></article>
      </section>
      ...
      <section className="card chart-card chart-card-wide">
        ...
        {branches.length === 0 ? <PageState type="empty" title={t('analytics.noOrderData')} /> : <><div className="chart-box chart-box-bar" aria-hidden="true"><Bar data={branchData} options={{ ...chartOptions, indexAxis: 'y' as const, scales: { x: { beginAtZero: true } } }} /></div><div className="responsive-table-wrap"><table className="data-table"><thead><tr><th>{t('analytics.branch')}</th><th>{t('analytics.orders')}</th><th className="text-right">{t('analytics.revenueShort')}</th></tr></thead><tbody>{branches.map((item: any) => <tr key={item.branch}><td data-label={t('analytics.branch')}>{item.branch}</td><td data-label={t('analytics.orders')}>{formatNumber(item.orders || 0)}</td><td data-label={t('analytics.revenueShort')} className="text-right">{formatNumber(item.revenue || 0)} ₸</td></tr>)}</tbody></table></div></>}
      </section>

      <section className="analytics-detail-grid">
        <article className="card table-card"><div className="table-heading"><div><h2>{t('analytics.topProducts')}</h2><p>{t('analytics.last30Days')}</p></div></div>{topProducts.length === 0 ? <PageState type="empty" title={t('analytics.noOrderData')} /> : <div className="responsive-table-wrap"><table className="data-table"><thead><tr><th>{t('analytics.product')}</th><th>{t('analytics.quantity')}</th><th className="text-right">{t('analytics.revenueShort')}</th></tr></thead><tbody>{topProducts.map((item: any) => <tr key={item.id || item.name}><td data-label={t('analytics.product')}>{item.name || item.id}</td><td data-label={t('analytics.quantity')}>{formatNumber(item.quantity || 0)}</td><td data-label={t('analytics.revenueShort')} className="text-right">{formatNumber(item.revenue || 0)} ₸</td></tr>)}</tbody></table></div>}</article>
        <article className="card funnel-card"><h2>{t('analytics.funnel')}</h2><p>{t('analytics.last30Days')}</p><ol>{[['app_open','analytics.funnelOpen'],['catalog_view','analytics.funnelCatalog'],['add_to_cart','analytics.funnelCart'],['checkout_started','analytics.funnelCheckout'],['payment_started','analytics.funnelPayment']].map(([key, label]) => <li key={key}><span>{t(label)}</span><strong>{formatNumber(Number(funnel[key] || 0))}</strong></li>)}</ol></article>
      </section>
    </div>
  );
}
