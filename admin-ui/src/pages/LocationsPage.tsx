...
export default function LocationsPage({ user }: { user: AdminUser | null }) {
  ...
  const selectCityPoint = (latitude: number, longitude: number, details?: MapPointDetails) => {
  ...
  const selectBranchPoint = (latitude: number, longitude: number, details?: MapPointDetails) => {
  ...
  const saveCity = async (event: FormEvent) => {
    ...
    if (!cityDraft.pointSelected || !Number.isFinite(latitude) || !Number.isFinite(longitude)) {
  ...
  const savePoint = async (event: FormEvent) => {
    ...
    if (
      !pointDraft.cityId ||
      pointDraft.name.trim().length < 2 ||
      pointDraft.address.trim().length < 3
    ) {
      setPointError(t('locations.pointValidation'));
      return;
    }
    if (!pointDraft.pointSelected || !Number.isFinite(latitude) || !Number.isFinite(longitude)) {
  ...
  if (error && locations.length === 0 && cities.length === 0)
  ...
  return (
    <div className="page-stack">
      ...
      {cityCounts.length === 0 ? (
        ...
        <>
          <section className="location-city-filter" aria-labelledby="locations-city-filter-title">
            ...
            <div
              className="segmented-control location-city-tabs"
              role="group"
              aria-label={t('locations.cityFilter')}
            >
              <button
                type="button"
                className={selectedCity === ALL_CITY_FILTER ? 'is-active' : ''}
                aria-pressed={selectedCity === ALL_CITY_FILTER}
                onClick={() => selectCity(ALL_CITY_FILTER)}
              >
                <span>{t('locations.allCities')}</span>
                <span className="location-city-count" aria-hidden="true">
                  {locations.length}
                </span>
              </button>
              {cityCounts.map((city) => (
                <button
                  type="button"
                  key={city.name}
                  className={selectedCity === city.name ? 'is-active' : ''}
          ...
          {visibleLocations.length === 0 ? (
            <PageState
              compact
              type="empty"
              title={t('locations.noPoints')}
              description={t('locations.noPointsHint', {
                city: selectedCity === ALL_CITY_FILTER ? t('locations.allCities') : selectedCity,
              })}
              action={
                canManageStructure ? (
                  <button
                ...
                <table className="data-table locations-table">
                  ...
                  <tbody>
                    {visibleLocations.map((location) => (
                      <tr key={location.id}>
                        <td data-label={t('locations.cityBranch')}>
                          <div className="location-name">
                            <MapPin aria-hidden="true" size={18} />
                            <div>
                              <strong>{location.name}</strong>
                              <small className="location-meta">{location.city}</small>
                            </div>
                          </div>
                        </td>
                        <td data-label={t('locations.address')}>
                          {location.address}
                        ...
                        <td data-label={t('locations.services')}>
                          <div className="location-statuses">
                            <span
                              className={`status-pill ${location.active ? 'status-active' : 'status-inactive'}`}
                            >
      ...
      <Modal
        open={cityOpen}
        onClose={() => !citySubmitting && setCityOpen(false)}
        title={t('locations.newCity')}
        size="lg"
      >
        <form className="modal-body form-stack" onSubmit={saveCity}>
          ...
          <div className="location-creation-intro">
            <Building2 aria-hidden="true" size={22} />
            <div>
              <strong>{t('locations.cityCreateStep')}</strong>
              <p>{t('locations.cityCreateHint')}</p>
            </div>
          </div>
          <div className="field-group">
            <label className="field-label" htmlFor="new-city-name">
            ...
            <input
              ...
              onChange={(event) =>
                setCityDraft((current) => ({ ...current, name: event.target.value }))
              }
          ...
          <fieldset className="form-section location-map-section">
            ...
            />
            <div
              className={`location-map-selection ${cityDraft.pointSelected ? 'is-selected' : ''}`}
              role="status"
            >
              <MapPin aria-hidden="true" size={18} />
              <span>
                {cityDraft.pointSelected
                  ? t('locations.mapPointSelected')
                  : t('locations.mapPointRequired')}
              </span>
            </div>
            <div className="form-grid form-grid-2 location-coordinate-grid">
              <div className="field-group">
                <label className="field-label" htmlFor="new-city-latitude">
                ...
                <input
                  ...
                  step="0.0000001"
                  className="input-classic"
                  value={cityDraft.latitude}
                  onChange={(event) =>
                    setCityDraft((current) => ({
                      ...current,
                      latitude: event.target.value,
                      pointSelected: Boolean(event.target.value && current.longitude),
                    }))
                  }
                  required
                />
              ...
              <div className="field-group">
                <label className="field-label" htmlFor="new-city-longitude">
                ...
                <input
                  ...
                  step="0.0000001"
                  className="input-classic"
                  value={cityDraft.longitude}
                  onChange={(event) =>
                    setCityDraft((current) => ({
                      ...current,
                      longitude: event.target.value,
                      pointSelected: Boolean(current.latitude && event.target.value),
                    }))
                  }
                  required
                />
          ...
          <div className="modal-actions">
            ...
            <button
              ...
              disabled={citySubmitting}
            >
              {citySubmitting && <LoaderCircle aria-hidden="true" className="spin" size={17} />}
              {citySubmitting ? t('common.saving') : t('locations.createCity')}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <Modal
        open={pointOpen}
        onClose={() => !pointSubmitting && setPointOpen(false)}
        title={t('locations.newPoint')}
        size="xl"
      >
        <form className="modal-body form-stack" onSubmit={savePoint}>
          ...
          <div className="location-creation-intro">
            ...
            <div>
              <strong>{t('locations.pointCreateStep')}</strong>
              <p>{t('locations.pointCreateHint')}</p>
            </div>
          </div>
          <div className="form-grid form-grid-2">
            <div className="field-group">
              <label className="field-label" htmlFor="new-point-city">
                {t('locations.cityName')}
              </label>
              <select
                id="new-point-city"
                className="input-classic"
                value={pointDraft.cityId}
                onChange={(event) =>
                  setPointDraft((current) => ({
                    ...current,
            ...
            <div className="field-group">
              ...
              <input
                ...
                onChange={(event) =>
                  setPointDraft((current) => ({ ...current, name: event.target.value }))
                }
            ...
            <div className="field-group location-form-wide">
              <label className="field-label" htmlFor="new-point-address">
                {t('locations.exactAddress')}
              </label>
              <input
                ...
                onChange={(event) =>
                  setPointDraft((current) => ({ ...current, address: event.target.value }))
                }
          ...
          <fieldset className="form-section location-map-section">
            ...
            <p className="page-help">
              {t('locations.pointMapHint', { city: pointCity?.name || '' })}
            </p>
            <YandexLocationMap
            ...
            />
            <div
              className={`location-map-selection ${pointDraft.pointSelected ? 'is-selected' : ''}`}
              role="status"
            >
              <MapPin aria-hidden="true" size={18} />
              <span>
            ...
            <div className="form-grid form-grid-2 location-coordinate-grid">
              <div className="field-group">
                <label className="field-label" htmlFor="new-point-latitude">
                  {t('locations.latitude')}
                </label>
                <input
                  id="new-point-latitude"
                  type="number"
                  step="0.0000001"
                  className="input-classic"
              ...
              <div className="field-group">
                <label className="field-label" htmlFor="new-point-longitude">
                  {t('locations.longitude')}
                </label>
                <input
                  id="new-point-longitude"
                  type="number"
                  step="0.0000001"
                  className="input-classic"
          ...
          <fieldset className="form-section">
            <legend>{t('locations.availability')}</legend>
            <div className="form-grid form-grid-2">
              {(
                [
                ...
                <label className="switch-row" key={key}>
                  <input
                    type="checkbox"
                    checked={pointDraft[key]}
                    onChange={(event) =>
                      setPointDraft((current) => ({ ...current, [key]: event.target.checked }))
                    }
                  />
      ...
      <Modal
        ...
        title={editing ? `${t('locations.settings')}: ${editing.name}` : t('locations.settings')}
        size="xl"
      >
