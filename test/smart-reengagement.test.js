...
test('smart reengagement uses the customer language and ordered product', () => {
  const automation = {
    ...
    body_translations: {
      ru: '{{quantity}} × {{productName}} снова ждут вас.',
      kk: '{{quantity}} × {{productName}} сізді қайта күтіп тұр.',
      en: '{{quantity}} × {{productName}} are waiting for you again.',
    },
  };
  const payload = {
    quantity: 3,
  ...
  assert.equal(renderAutomationCopy(automation, payload, 'ru').body, '3 × Плюшка снова ждут вас.');
  assert.equal(
    renderAutomationCopy(automation, payload, 'kk').body,
    '3 × Тоқаш сізді қайта күтіп тұр.',
  );
  assert.equal(
    renderAutomationCopy(automation, payload, 'en').body,
    '3 × Sweet bun are waiting for you again.',
  );
