...
function database(rows, error, customRows = []) {
  const schemas = {
    menu_overrides: ['iiko_product_id', 'custom_image_url', 'name_translations'],
    ...
    from: (table) => ({
      select: (columns) => ({
        in: async (key, ids) => {
          ...
          return invalid
            ? { error: new Error(`column ${table}.${invalid} does not exist`) }
            : {
                data: (table === 'menu_overrides' ? rows : customRows).filter((row) =>
...
test('historical names translate even when a photo already exists, preserving the original sale', async () => {
  const item = {
    id: 'bun',
  ...
  const result = await attachOrderImages([{ amount: 70, items: [item] }], { db: database([]) });
  const translated = result[0].items[0];
  assert.ok(translated.name_translations.kk);
  assert.match(translated.name_translations.en, /bun/i);
  assert.equal(translated.name, item.name);
  assert.equal(translated.price, 35);
  assert.equal(translated.quantity, 2);
  assert.equal(translated.imageUrl, item.imageUrl);
  assert.equal(result[0].amount, 70);
  assert.equal(item.name_translations, undefined);
});
...
test('purchase images enrich missing photos without changing historical prices or existing photos', async () => {
  ...
  const result = await attachOrderImages(orders, {
    db: database([
...
test('conflicting branch images and unavailable storage never hide purchases', async () => {
  ...
  assert.equal(
    await attachOrderImages(orders, { db: database([], new Error('unavailable')) }),
    orders,
  );
});
