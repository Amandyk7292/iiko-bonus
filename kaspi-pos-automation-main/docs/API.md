...
| ----------------- | -------- | ------------ | ---------------------------------------- |
| `X-Token-SN`      | `string` | да           | Токен сессии, полученный при авторизации |
| `X-Vtoken-Secret` | `string` | да           | Зашифрованный секрет сессии              |
| `X-Profile-Id`    | `string` | нет          | ID профиля организации                   |
...
> Важно: **Важно:** Для входа используйте номер телефона аккаунта **кассира** Kaspi Pay.
...
| ------------- | -------- | ------------ | -------------------------------------- |
| `phoneNumber` | `string` | да           | Номер телефона (формат: `7XXXXXXXXXX`) |
| `processId`   | `string` | да           | ID процесса из `/api/auth/init`        |
...
> Важно: Сохраните `tokenSN` и `vtokenSecret` — они нужны для всех последующих запросов.
...
| Поле      | Тип      | Обязательный | Описание     |
| --------- | -------- | ------------ | ------------ |
| `tokenSN` | `string` | нет          | Токен сессии |
...
> Защищено: Все эндпоинты требуют [заголовки сессии](#заголовки-сессии).
...
| `comment`     | `string` | нет          | Комментарий к платежу  |
...
| Параметр      | Тип      | Обязательный | Описание    |
| ------------- | -------- | ------------ | ----------- |
| `operationId` | `string` | да           | ID операции |
...
| Поле          | Тип      | Обязательный | Описание               |
| ------------- | -------- | ------------ | ---------------------- |
| `operationId` | `string` | да           | ID операции для отмены |
...
| ----------- | -------- | ------------ | ------------------------------ |
| `amount`    | `number` | да           | Сумма в тенге                  |
| `latitude`  | `number` | нет          | Широта (по умолчанию: Алматы)  |
| `longitude` | `number` | нет          | Долгота (по умолчанию: Алматы) |
...
> Совет: `QrToken` содержит ссылку для оплаты — можно преобразовать в QR-код.
...
| Параметр        | Тип      | Обязательный | Описание                           |
| --------------- | -------- | ------------ | ---------------------------------- |
| `qrOperationId` | `string` | да           | ID QR-операции из `/api/qr/create` |
...
| Поле                  | Тип      | Обязательный | Описание                                  |
| --------------------- | -------- | ------------ | ----------------------------------------- |
| `endDate`             | `string` | да           | Конечная дата (формат: `YYYY-MM-DD`)      |
| `lastTransactionDate` | `string` | нет          | Дата последней транзакции (для пагинации) |
| `statementPeriodCode` | `number` | нет          | Код периода (по умолчанию: `0`)           |
...
| Поле              | Тип      | Обязательный | Описание                           |
| ----------------- | -------- | ------------ | ---------------------------------- |
| `id`              | `number` | да           | ID операции                        |
| `operationMethod` | `number` | нет          | Метод операции (по умолчанию: `0`) |
...
| Поле            | Тип      | Обязательный | Описание                 |
| --------------- | -------- | ------------ | ------------------------ |
| `qrOperationId` | `number` | да           | ID операции для возврата |
| `returnAmount`  | `number` | да           | Сумма возврата в тенге   |
...
> Защищено: Требует [заголовки сессии](#заголовки-сессии).
...
| Поле     | Тип        | Обязательный | Описание                                       |
| -------- | ---------- | ------------ | ---------------------------------------------- |
| `url`    | `string`   | да           | URL, на который будут отправляться уведомления |
| `events` | `string[]` | да           | Список событий для подписки                    |
| `secret` | `string`   | нет          | Секрет для HMAC-подписи (рекомендуется)        |

> Совет: Для начала скопируйте `webhooks.example.json` → `webhooks.json` и отредактируйте.
